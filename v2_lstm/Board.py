...
class Board:
    ...
    def play_until_end(self, next_move):
        ...
        while not b.game_over():
            # We have to make a temporary copy of BoardSequence in order to call next_move.
            # Note that we should not call our neural net's next_move function here,
            # since it depends on a large history of moves rather than just the one.
            seq = BoardSequence(b)
            p = next_move(seq)
            b.update(p)
    ...
    def evaluate(self, next_move):
        """
        A function to evaluate a board using a tree search
        and random play.
